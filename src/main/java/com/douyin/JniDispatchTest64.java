...
public class JniDispatchTest64 extends AbstractJni {
    ...
    JniDispatchTest64() throws IOException {
        ...
        vm.setJni(this);
        vm.setVerbose(true);
        DalvikModule dm = vm.loadLibrary(new File("src/main/resources/example_binaries/armeabi-v7a/libcms.so"), false);
...
//        Number ret = __system_property_get.call(emulator, "ro.build.version.sdk", block.getPointer())[0];
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int j = 0; j < 100; j++) {
            JniDispatchTest64 test = new JniDispatchTest64();
            for (int i = 0; i < 100; i++) {
//            Thread.sleep(1000);
                test.leviathan((int) (System.currentTimeMillis() / 1000), data);
//            test.test();
    ...
    public byte[] leviathan(int time, byte[] data) {
        Number ret = Native.callStaticJniMethod(emulator, "leviathan(II[B)[B", -1, time, new ByteArray(data));
